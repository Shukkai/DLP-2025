...
class Test_model(VAE_Model):
    def __init__(self, args):
        ...
        self.writer = SummaryWriter(log_dir=os.path.join(self.args.save_root, 'logs'))
    ...
    def val_one_step(self, img, label, idx=0):
        ...
        for i in range(label.shape[0]-1):
            img_in = self.frame_transformation(img_in)
            label_in = self.label_transformation(label[i])
            z, mu, logvar = self.Gaussian_Predictor(img_in,label_in)
            z = mu
            output = self.Decoder_Fusion(img_in,label_in,z)
